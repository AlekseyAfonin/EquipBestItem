...
namespace EquipBestItem.Settings
{
    public class Filters
    {
        ...
        public Filters()
        {
            FilterElement = new FilterElement[12];

            for (var i = 0; i < 12; i++) FilterElement[i] = new FilterElement();

            for (var index = EquipmentIndex.WeaponItemBeginSlot; index < EquipmentIndex.NumEquipmentSetSlots; index++)
                if (index is >= EquipmentIndex.WeaponItemBeginSlot and < EquipmentIndex.NumAllWeaponSlots)
                {
        ...
        public FilterElement this[EquipmentIndex index] => FilterElement[(int) index];

        public bool IsDefault(EquipmentIndex index, bool isInWarSet)
        {
            var defaultFilters = SettingsLoader.Instance.GetCharacterSettingsByName("default_equipbestitem", isInWarSet)
                .Filters;
            ...
            if (index is >= EquipmentIndex.WeaponItemBeginSlot and < EquipmentIndex.NumAllWeaponSlots)
            {
                ...
                if (Math.Abs(this[index].WeaponBodyArmor - defaultFilters[index].WeaponBodyArmor) > Tolerance)
                    return false;
            }
            else if (index is >= EquipmentIndex.ArmorItemBeginSlot and < EquipmentIndex.ArmorItemEndSlot)
            {
                if (Math.Abs(this[index].HeadArmor - defaultFilters[index].HeadArmor) > Tolerance) return false;
                if (Math.Abs(this[index].ArmorBodyArmor - defaultFilters[index].ArmorBodyArmor) > Tolerance)
                    return false;
